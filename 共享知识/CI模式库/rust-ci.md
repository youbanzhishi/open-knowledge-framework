...
## macOS 交叉编译模式（2026-06-12 沉淀）

> **核心经验**：同组织项目已有成功先例时直接复用，不要从零摸索。

### 问题

`macos-13`（Intel runner）排队严重，免费账户经常排队1小时+甚至卡死。

### 解法

只用 `macos-latest`（ARM runner，秒启）+ 交叉编译产出 Universal Binary：

```yaml
# macOS 构建
- name: Build macOS
  if: runner.os == 'macOS'
  run: cargo build --release --target universal-apple-darwin
```

### Rust vs CMake 交叉编译对照

| 项目类型 | 交叉编译配置 | 先例 |
|---------|-------------|------|
| Rust (cargo) | `--target universal-apple-darwin` | OpenDAW desktop.yml 已验证 |
| CMake (C++/JUCE) | `CMAKE_OSX_ARCHITECTURES='x86_64;arm64'` | AudioFX build.yml 已验证 |

### 决策逻辑

1. 同组织有成功先例 → 直接复用方案
2. 交叉编译 vs 等 Intel runner：秒启 vs 排队1小时+，没有可比性
3. Universal Binary 兼容性更好：一个包同时支持 Intel + Apple Silicon
